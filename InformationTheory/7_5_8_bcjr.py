...
class BCJR_4States():
    ...
    def state_transfer(self, input, state):
        uu1 = int(self.states[state][0])
        next_state = f'{input}{uu1}'
        return self.states.index(next_state)

    def encode_state(self, input, state):
        codeword = np.empty([0], order='C')
        uu1 = int(self.states[state][0])
        uu2 = int(self.states[state][1])
        cc1 = (input + uu1 + uu2) % 2
        cc2 = (input + uu2) % 2
        codeword = np.append(codeword, [cc1, cc2])
        return codeword
...
if __name__ == '__main__':
    ...
    FILE_PATH = './result/'
    board_dir = '75bcjr_dec'
    results.to_csv(FILE_PATH + board_dir + ".csv")

    plt.figure()
    plt.semilogy(noise_sigma, BER, 'o-', label='Conv Code $(7,5)_8$ & bcjr Decoder')
    ...
    plt.legend()
    plt.show()
    plt.savefig('plot_(7, 5)bcjr.png')