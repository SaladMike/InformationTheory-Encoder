...
'''def conv_encoder1513(xx):  ## (15, 13)_8
    coded_bits = np.empty([0], order='C')
    # 两个寄存器u_1 u_2初始化为0
    uu1, uu2, uu3, uu4, uu5, uu6, uu7 = 0, 0, 0, 0, 0, 0, 0
    for jj in xx:
        c_1 = (jj + uu1 + uu2 + uu4 + uu5 + uu7) % 2
        c_2 = (jj + uu1 + uu3 + uu4 + uu6 + uu7) % 2
        c_3 = (jj + uu2 + uu3 + uu5 + uu6 + uu7) % 2
        coded_bits = np.append(coded_bits, [c_1, c_2, c_3])
        # 更新寄存器
        uu7 = uu6
        uu6 = uu5
        uu5 = uu4
...
class Soft_Viterbi_4States():
    def __init__(self, in_seq):
        ...
        self.accu_weight = np.concatenate((np.zeros((4,1)), self.accu_weight), axis=1 )
    ...
    def state_transfer(self, input, now_state):
        u_1 = int(self.states[now_state][0]) # 15 13 是三位二进制数
        next_state = f'{input}{u_1}'
        return self.states.index(next_state)

    def encode_state(self, xx, state): # 需要修改
        codeword = np.empty([0], order='C')
        uu1 = int(self.states[state][0])
        uu2 = int(self.states[state][0])
        uu3 = int(self.states[state][1])
        c_1 = (xx + uu1 + uu3) % 2
        c_2 = (xx + uu2 + uu3) % 2
...
if __name__ == '__main__':
    ...
    FILE_PATH = './result/'
    board_dir = 'soft_viterbi'
    results.to_csv(FILE_PATH + board_dir + ".csv")

    plt.figure()
    plt.semilogy(noise_sigma, BER, 'o-', label='Conv Code $(7,5)_8$ & Soft Viterbi Decoder')
