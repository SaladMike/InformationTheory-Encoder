...
class Soft_Viterbi_4States():
    def __init__(self, in_seq):
        ...
        self.accu_weight = np.array([np.inf * np.ones(num_words) for _ in range(64)])
        aa = np.inf * np.ones((64,1))
        aa[0] = 0
    ...
    def decode(self):
        ...
        self.trace_block = [[-1,-1] for _ in range(64)]
        ...
        for idx in range(2, int(len(self.demod_bits)), 2):
            ...
            self.trace_back_list.append(self.trace_block)
            self.trace_block = [[-1,-1] for _ in range(64)]  # 在每次处理新的 demod_bits（即一对接收到的符号）时重置 trace_block
            pass
        # 完成前向，开始回溯
...
if __name__ == '__main__':
    ...
    FILE_PATH = './result/'
    board_dir = '171soft_viterbi'
    results.to_csv(FILE_PATH + board_dir + ".csv")

    plt.figure()
    plt.semilogy(noise_sigma, BER, 'o-', label='Conv Code $(171,133)_8$ & Soft Viterbi Decoder')
    ...
    plt.legend()
    plt.show()
    plt.savefig('plot_(171, 133)s.png')